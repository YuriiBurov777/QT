...
private slots:
    ...
    // Новый слот для отображения графика
    void onChartDataReady(const QVector<double>& chartData);

signals:
    // Сигнал, испускаемый когда данные готовы для отрисовки
    void chartDataReady(const QVector<double>& chartData);
